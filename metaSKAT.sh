...
for gene in "${genelist[@]}"; do
    ...
    Rscript MetaSKAT.r "$gene" "${cohorts[@]}" 2>&1 | tee "METASKAT/$gene/MetaSKAT.$gene.log"
    
    # Parallelize per gene:
    # command="module load StdEnv/2020 nixpkgs/16.09 gcc/7.3 r/3.5.2 && Rscript MetaSKAT.r  $gene ${cohorts[@]} 2>&1 | tee 'METASKAT/$gene/MetaSKAT.$gene.log' && echo 'Done!'"
...
# echo "ANO3_AM" >> METASKAT/logs/singleton_and_empty_sets.txt # Manual exclusion
grep -wv -f METASKAT/logs/singleton_and_empty_sets.txt "METASKAT/results.metaskato.raw" | awk '$2 != 1' > "METASKAT/results.metaskato"
# ---------------------------------------
...
join -a 1 METASKAT/logs/set_lengths.txt <(join -a 1 METASKAT/results.metaskato.raw METASKAT/results.metaskato.adjusted.txt -o 0,0,1.2,2.5) > METASKAT/manualcheck.txt; echo "EXCLUDED SETS: \n" >> METASKAT/manualcheck.txt; grep -vf <(cut -d" " -f1 METASKAT/results.metaskato.adjusted.txt) <(cut -d" " -f1 METASKAT/results.metaskato.raw)  >> METASKAT/manualcheck.txt
echo "Done! Your results are in METASKAT/results.metaskato.adjusted.txt"
