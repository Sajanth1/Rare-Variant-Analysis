...
# 1. Create subsets of bfiles for each gene (feeding entire bed file seems to create issues with SKAT)
for gene in "${genelist[@]}"; do
  #rm -r "SKAT/$gene" "logs/SKAT_${gene}.log"
  mkdir "SKAT/$gene"
  plink2 --bfile $cohort --double-id --extract <(cut -f2 "SKAT/$cohort.$gene.SETID") --make-bed --out "SKAT/$gene/$cohort.$gene" --silent
done
